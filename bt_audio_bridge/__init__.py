...
CONF_DEVICE = "device"
CONF_CURRENT_SPEAKER = "current_speaker"
...
CONFIG_SCHEMA = cv.Schema({
    ...
    cv.Optional(CONF_CURRENT_SPEAKER): text_sensor.text_sensor_schema(),
...
async def to_code(config):
    ...
    if current_speaker_config := config.get(CONF_CURRENT_SPEAKER):
        sensor = await text_sensor.new_text_sensor(current_speaker_config)
        cg.add(var.set_current_speaker_sensor(sensor))
