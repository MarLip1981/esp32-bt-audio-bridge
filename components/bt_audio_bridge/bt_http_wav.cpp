...
void BtAudioBridge::play_http_wav() {
  if (this->engine_test_active_ || this->http_wav_requested_) {
    ESP_LOGW(HTTP_TAG, "HTTP WAV playback already active/queued");
  ...
  log_heap("before HTTP request signal");
  this->http_wav_requested_ = true;
  this->publish_event_("HTTP WAV: download queued");
}
...
void BtAudioBridge::http_wav_playback_() {
  ...
  log_heap("after static audio buffer init");
